...
public class LocationGateway implements LocationResolver {
  ...
  public Location resolveByIdentifier(String identifier) {
    if (StringUtils.isEmpty(identifier)) {
      throw new LocationGatewayException("Identifier cannot be null or empty", 400);
    }
    return locations.stream()
        .filter(location -> location.identification.equals(identifier))
        .findFirst()
        .orElseThrow(() -> new LocationGatewayException(String.format("Location with identifier %s not found", identifier), 404));
  }
}
